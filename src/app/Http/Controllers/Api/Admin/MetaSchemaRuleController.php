...
class MetaSchemaRuleController extends ApiController
{
    protected $repository;
    protected $entity;

    public function __construct(MetaSchemaRuleRepository $repository)
    {
        ...
        if (isset(config('meta.transformers')['meta'])) {
            ...
            $this->transformer = MetaSchemaRuleTransformer::class;
        }
    }
    ...
    public function index(Request $request)
    {
        ...
        if ($request->has('page')) 
        {
            ...
            $meta_schema_rules = $query->paginate($per_page);
    
            return $this->response->paginator($meta_schema_rules, new $this->transformer());
        }

        $meta_schema_rules = $query->get();

        return $this->response->collection($meta_schema_rules, new $this->transformer());
    }
}
